...
describe("data export route", () => {
  ...
  it("enforces auth, tenant, module, and body gates", async () => {
    ...
    expect((await disabled.app.inject({ method: "POST", url: "/v1/tenants/t1/dsar", headers: { authorization: `Bearer ${disabled.token}` }, payload })).json()).toMatchObject({ error: "module_not_enabled" });
