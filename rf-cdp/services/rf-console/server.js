...
// 3 честных триггера (не все ~33 из деки — это отдельный долгий бэклог):
...
async function sendAutomationEmail(tenant, trigger, subject, email, subjectId, fromName, fromEmail) {
  ...
  const html = trigger === 'abandoned_cart' ? abandonedCartEmailHtml() : trigger === 'abandoned_browse' ? abandonedBrowseEmailHtml() : reactivationEmailHtml();
...
async function runAutomationPoller(tenant) {
  ...
  const out = { abandoned_cart: { checked: 0, sent: 0, failed: 0 }, abandoned_browse: { checked: 0, sent: 0, failed: 0 }, reactivation: { checked: 0, sent: 0, failed: 0 } };
