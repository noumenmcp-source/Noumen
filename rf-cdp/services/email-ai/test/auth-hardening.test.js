'use strict';
/**
 * Auth-hardening (Phase 3) + rate-limit (Phase 4) wiring for email-ai.
...
test('revoked token -> 401', async () => {
  await withServer({ EMAIL_API_TOKEN: 'adm', EMAIL_TENANT_TOKENS: 'aero:tA', EMAIL_REVOKED_TOKENS: 'tA' }, async ({ req }) => {
    const r = await req('/v1/campaign/preview', { token: 'tA', method: 'POST', body: JSON.stringify({ site: 'aero' }) });
...
test('admin introspection ok; tenant token forbidden (403)', async () => {
  await withServer({ EMAIL_API_TOKEN: 'adm', EMAIL_TENANT_TOKENS: 'aero:tA' }, async ({ req }) => {
    const a = await req('/v1/auth/introspect', { token: 'adm', method: 'POST', body: JSON.stringify({ token: 'tA' }) });
    assert.equal(a.status, 200);
    assert.equal((await a.json()).kind, 'tenant');
...
test('rate limit: 3rd request 429 (cap=2); /v1/live unaffected', async () => {
  await withServer({ EMAIL_API_TOKEN: 'adm', EMAIL_RATE_CAPACITY: '2', EMAIL_RATE_REFILL_PER_SEC: '0' }, async ({ req }) => {
    await req('/v1/campaign/preview', { token: 'adm', method: 'POST', body: '{}' }); // 400 but consumes
    await req('/v1/campaign/preview', { token: 'adm', method: 'POST', body: '{}' });
    assert.equal((await req('/v1/campaign/preview', { token: 'adm', method: 'POST', body: '{}' })).status, 429);
